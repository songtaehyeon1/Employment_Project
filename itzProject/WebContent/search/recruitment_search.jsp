...
<script>
	$(function(){
		...
		$("#big_search ul:nth-of-type(2) ~ ul input:checkbox, #big_search_2 input:checkbox").click(function(){
			...
			str = str.replace("·", "");
			str = str.replace(",", "");
			var idStr = $(this).attr("id").substring(0, $(this).attr("id").indexOf("_"));
			
			// 체크 되었을 때
			if($(this).prop("checked")){
				$("#tags>ul").append("<li id = 'tag_" + str + "'><button class = 'btn btn-primary' value = '"
						+ $(this).attr("id") + "' name = '" + $(this).val() + "'>" + $(this).val() 
						+ "<span>×</span></button><input type = 'hidden' name = 'asd' value = '" + str + "/" + idStr + "'></li>");
			}else{	// 체크 풀었을 때
		...
		$("#age_btn").click(function(){
			...
			if($("#age_text").val() == ""){
				...
				$("#tags>ul").append("<li id = 'tag_age'><button class = 'btn btn-primary' value = '"
						...
						+ "세<span>×</span></button><input type = 'hidden' name = 'asd' value = '" + $("#age_text").val() + "/" + idStr + "'></li>");
			}
		});
		...
		$("#gender_check").click(function(){
			...
			if($("#tags #tag_gender")){
				...
				$("#tags>ul").append("<li id = 'tag_gender'><button class = 'btn btn-primary' value = '"
					+ gender_check + "' name = 'gender'>" + gender_check 
					+ "<span>×</span></button><input type = 'hidden' name = 'asd' value = '" + gender_check + "/" + idStr + "'></li>");
			}
		});
...
<div style = "width : 100%;">
	<div class = "container-fuild" style = "width : 100%; height : 115px;">
		...
		<div id = "bar" class = "bg-primary">
			<div id="search_bar">
				<a href = "<%= ctx %>/search/recruitment_search.do" style = "font-weight : bold; font-size : 1.5em;">채용 검색</a>
	...
	<div class = "container" id = "center">
		...
		<!-- 태그 -->
		<div id = "tag_search">
			<form action = "tagSearch.do" method = "post">
				<div id = "tags">
					<ul>
					</ul>
				</div>
				<div id = "tag_reset"><button type="button" class = "btn btn-primary">초기화</button></div>
					...
					<li class = "border border-primary ml-1 mb-1">
						<a href = "../jobs_user_info/companyHire.html" target = "_black">
							<span class = "slideLogo">
								<c:if test = "${vo.re_img == null || vo.re_img == ''}"><img src = "<%= ctx %>/upload/re_undefoned.jpg"></c:if>
								<c:if test = "${vo.re_img != null}"><img src = "<%= ctx %>/upload/${vo.re_img}"></c:if>
							</span>
							<strong>${vo.re_name}</strong>
							<span>${vo.re_title}</span>
						</a>
					</li>
				</c:forEach>
			</ul>
					...
					<div class = "row">
						...
						<div class = "col pt-4">
							<div class = "pb-2 font-weight-bold">
								<a href = "../jobs_user_info/companyHire.html" target = "_black">${vo.re_title}</a>
							</div>
							<div class = "pb-1">
								<c:if test = "${vo.re_rfcareerstatus != null || vo.re_rfcareerstatus != ''}">${vo.re_rfcareerstatus}&emsp;</c:if>
								<c:if test = "${vo.re_qcacademicbackground != null || vo.re_qcacademicbackground != ''}">${vo.re_qcacademicbackground}&emsp;</c:if>
								<c:if test = "${vo.re_ciwa != null || vo.re_ciwa != ''}">${vo.re_ciwa}&emsp;</c:if>
								<c:if test = "${vo.re_qcservicetype != null || vo.re_qcservicetype != ''}">${vo.re_qcservicetype}&emsp;</c:if>
							</div>
							<div class = "pt-4">뭘써야되는지 모름</div>
						</div>
